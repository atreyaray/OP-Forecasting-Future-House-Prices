...
A reliable and easy-to-use solution for those who want to earn with investing but have no time or knowledge with investing
...
def preprocess_input(postal_code, housing_type):
    ...
    latitude, longitude = geolocation["latitude"].iloc[0], geolocation["longitude"].iloc[0]
...
def user_input_features():
    ...
    housing_type = st.sidebar.selectbox('Housing type',('one-room','two-room','three or more room', 'terrace house'))
...
st.subheader('Prediction price (EUR/m2) for 2021 Q2')
st.write(int(pred))